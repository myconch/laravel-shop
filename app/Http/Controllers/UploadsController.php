...
{
    // 图片保存在 laravel-shop\public\uploads\images
    public function uploadImg(Request $request)
    {
        $file = $request->file("mypic");
        $name = $request->session()->get("_previous");
        //$nameS = $name->get("_previous")["url"];
        $i = $request;
        if (!empty($file)) {
            ...
            for ($i = 0; $i <= $len;$i++) {
                ...
                if ($file[$i]->isValid()) {
                    if (in_array(strtolower($file[$i]->extension()),['jpeg','jpg','gif','gpeg','png'])) {
                        ...
                        return response()->json(['ResultData' => 3,'info' => '第'.$n.'张图片后缀名不合法！<br/>'.'只支持jpeg/jpg/gif/gpeg/png']);
                    }
                } else {
